export default function pizzaData(db) {
  ...
  async function myCartItems(sessionId){
    try{
      return await db.manyOrNone('select catalogue.id,product,price,catalogue.qty,size,image_url from products join catalogue on products.id = catalogue.product_id where session_id =$1',[sessionId]);

    }catch(err){
      console.log(err)
    }
  }
  async function updateItemQty(newValue,id){
    try{
      ...
      let productPrices = await db.oneOrNone('select small_price,medium_price,large_price from products join catalogue on products.id = catalogue.product_id where catalogue.id=$1',[id]);
